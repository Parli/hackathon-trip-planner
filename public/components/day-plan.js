...
class DayPlan extends HTMLElement {
  ...
  set activities(activities) {
    ...
    this._hasTransportation = activities.some(activity => activity.kind === 'transportation');
    this.render();
  }
  ...
  _formatDate(timestamp) {
    ...
    formattedDate = formattedDate.replace(/\d+$/, `${day}${suffixes[relevantDigits] || suffixes[0]}`);
  ...
  render() {
    ...
    this.shadowRoot.innerHTML = `
      ...
      <div class="header">
        ...
        <div class="icons">
          ${this._hasTransportation ? '<transit-icon mode="plane"></transit-icon>' : ''}
          ${this._weather ? `<weather-icon condition="${this._weather.condition}"></weather-icon>` : ''}
        </div>
        
        ${this._weather ? `<div class="temp">${this._getTemperatureRange()}</div>` : ''}
      </div>
    ...
    sortedActivities.forEach(activity => {
      const element = this.shadowRoot.getElementById(`activity-${activity.start_time}`);
      if (element) {
        if (activity.kind === 'transportation') {
